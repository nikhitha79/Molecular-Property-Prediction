...
def objective(trial):
    ...
    def get_xyz_files(directory):
        ...
        # Sort files by Roman nutrainal first, then by the integer part
    ...
    # Now `schnet_compatible_data` contains grouped 'atomic_numbers' and 'positions' in nested lists
    ...
    # dpm_test = test['dipole_moments'].tolist()
    # dpm_all = dpm_train 
    ...
    # Define sizes
    total_data = len(train)  # or however you're defining the full dataset
    total_val = 3000
    total_test = 1500
    ...
    # Create splits from shuffled indices
    ...
    # Load the existing split file
    split_file_path = 'split_file2.json'  # Make sure to provide the correct path to your split file
    ...
    # Suggest a learning rate to try
    ...
    # Create the task with the suggested learning rate
    task = spk.task.AtomisticTask(
        model=nnpot,
        outputs=[output_corr],
        optimizer_cls=torch.optim.AdamW,
        optimizer_args={'lr': lr},  # Set the learning rate here
...
# Create the Optuna study and optimize without pruning
